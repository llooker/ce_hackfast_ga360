...
view: user_facts {
  derived_table: {
    ...
    unique_hour_of_day AS(
      (SELECT   ga_sessions_visit_start_hour_of_day, fullvisitorId FROM (SELECT ROW_NUMBER () OVER(PARTITION BY fullvisitorId ORDER BY pageviews DESC) as row_number, fullvisitorId, ga_sessions_visit_start_hour_of_day
      ...
      unique_dma AS(
      (SELECT   metro, fullvisitorId FROM (SELECT ROW_NUMBER () OVER(PARTITION BY fullvisitorId ORDER BY pageviews DESC) as row_number, fullvisitorId, metro
      ...
      (SELECT   ga_sessions_visit_start_day_of_week, fullvisitorId FROM (SELECT ROW_NUMBER () OVER(PARTITION BY fullvisitorId ORDER BY pageviews DESC) as row_number, fullvisitorId, ga_sessions_visit_start_day_of_week
